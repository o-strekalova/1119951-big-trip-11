...
const createEditFormTemplate = (tripEvent) => {
  ...
  let startTime = deconstructDate(start);
  let {year: startYear, month: startMonth, day: startDay, hours: startHours, minutes: startMinutes} = startTime;
  let finishTime = deconstructDate(finish);
  let {year: finishYear, month: finishMonth, day: finishDay, hours: finishHours, minutes: finishMinutes} = finishTime;
  ...
  for (let i = 0; i < offers.length; i++) {
    ...
    offersList.insertAdjacentHTML(`beforeend`, `<div class="event__offer-selector">
      <input class="event__offer-checkbox  visually-hidden" id="event-offer-${offersTypes[offerTitle]}-1" type="checkbox" name="event-offer-${offersTypes[offerTitle]}" checked>
      <label class="event__offer-label" for="event-offer-${offersTypes[offerTitle]}-1">
